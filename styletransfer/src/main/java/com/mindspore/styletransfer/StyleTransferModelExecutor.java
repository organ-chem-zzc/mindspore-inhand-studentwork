...
public class StyleTransferModelExecutor {
    ...
    // 存储用户自定义模型路径（进程级别、跨 recreate 保留）
    private static String sPredictModelPath  = null;
    private static String sTransferModelPath = null;
    ...
    public void init() {
        ...
        // 优先用用户自定义，其次回退 assets
        String predictPath  = (sPredictModelPath  != null) ? sPredictModelPath  : "style_predict_quant.ms";
        String transferPath = (sTransferModelPath != null) ? sTransferModelPath : "style_transfer_quant.ms";


        style_predict_model = new Model();
        boolean okPredict  = predictPath.startsWith("/")  // 粗判：包含 / 认为是文件路径
                ? style_predict_model.loadModel(predictPath)
                : style_predict_model.loadModel(mContext, predictPath);

        style_transform_model = new Model();
        boolean okTransfer = transferPath.startsWith("/")
                ? style_transform_model.loadModel(transferPath)
                : style_transform_model.loadModel(mContext, transferPath);
    ...
    public static boolean setPredictModelPath(String path) {
        Model test = new Model();
        if (!test.loadModel(path)) {          // 先验证能否正常反序列化
            return false;
        }
        test.free();
        sPredictModelPath = path;
        return true;
    }

    public static boolean setTransferModelPath(String path) {
        Model test = new Model();
        if (!test.loadModel(path)) {
            return false;
        }
        test.free();
        sTransferModelPath = path;
        return true;
    }

    // 新增加载自定义模型的函数
    public boolean loadCustomPredictModel(String modelPath) {
        // 释放旧的预测 Session
        if (style_predict_model != null) style_predict_model.free();
        style_predict_model = new Model();
        return style_predict_model.loadModel(modelPath);
    }

    public boolean loadCustomTransferModel(String modelPath) {
        if (style_transform_model != null) style_transform_model.free();
        style_transform_model = new Model();
        return style_transform_model.loadModel(modelPath);
    }
