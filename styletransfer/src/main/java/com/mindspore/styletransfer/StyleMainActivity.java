...
public class StyleMainActivity extends AppCompatActivity implements OnBackgroundImageListener {
    ...
    // 新增选择两个模型的按钮
    private static final int RC_PICK_PREDICT_MODEL  = 101;
    private static final int RC_PICK_TRANSFER_MODEL = 102;
    ...
    private void showMsgDialog(String msg) {
        noticeDialog = new NoticeDialog(this);
        noticeDialog.setTitleString("Debug Message");
        noticeDialog.setContentString(msg);
        noticeDialog.setYesOnclickListener(() -> {
            noticeDialog.dismiss();
        });
        noticeDialog.show();
    ...
    public void onClickPickPredict(View v) {
        openModelPicker(RC_PICK_PREDICT_MODEL);
    }

    public void onClickPickTransfer(View v) {
        openModelPicker(RC_PICK_TRANSFER_MODEL);
    }

    private void openModelPicker(int requestCode) {
        Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT);
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        intent.setType("*/*");                       // MindSpore .ms 文件一般是 application/octet-stream
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            if (RC_CHOOSE_PHOTO == requestCode) {
                ...
                showOriginCamera();
            } else if (RC_PICK_PREDICT_MODEL == requestCode) {
//                showMsgDialog("PICK_PREDICT_MODEL");
                if (data != null && data.getData() != null) {
                    File dst = copyUriToFile(data.getData(), "custom_predict.ms");
                    boolean ok = dst != null && transferModelExecutor.loadCustomPredictModel(dst.getAbsolutePath());
                    Toast.makeText(this, ok ? "已加载预测模型" : "预测模型加载失败", Toast.LENGTH_SHORT).show();
                    if(ok){
                        StyleTransferModelExecutor.setPredictModelPath(dst.getAbsolutePath());
                        recreate();
                    }
                }
            } else if (RC_PICK_TRANSFER_MODEL == requestCode) {
//                showMsgDialog("PICK_TRANSFER_MODEL");
                if (data != null && data.getData() != null) {
                    File dst = copyUriToFile(data.getData(), "custom_transfer.ms");
                    boolean ok = dst != null && transferModelExecutor.loadCustomTransferModel(dst.getAbsolutePath());
                    Toast.makeText(this, ok ? "已加载变换模型" : "变换模型加载失败", Toast.LENGTH_SHORT).show();
                    if(ok){
                        StyleTransferModelExecutor.setTransferModelPath(dst.getAbsolutePath());
                        recreate();
                    }
                }
            }
    ...
    private File copyUriToFile(Uri uri, String fileName) {
        File dst = new File(getFilesDir(), fileName);   // 私有目录 /data/user/0/包名/files/
